...
class Project:
    ...
    def get_all_projects():
        ...
        try:
            cur = conn.cursor()
            cur.execute(
                "SELECT id, title, description, image_url, technologies, github_link, live_demo_link, date_added FROM projects ORDER BY date_added DESC"
            )
    ...
    def get_project_by_id(project_id):
        ...
        try:
            cur = conn.cursor()
            cur.execute(
                "SELECT id, title, description, image_url, technologies, github_link, live_demo_link, date_added FROM projects WHERE id = %s",
                (project_id,),
            )
            row = cur.fetchone()
            if row:
